...
var e={};return d(b,function(a,d){e[d]=e[d]||0,e[d]++,c[d]>1&&(b[a]=d+" "+e[d])}),b},getOption:function(a){try{throw"getOption is deprecated"}catch(b){window.console.error(b)}return this.options[a]},showValidationErrors:function(a){}}),f.defaults.editors["null"]=f.AbstractEditor.extend({getValue:function(){return null},setValue:function(){this.onChange()},getNumColumns:function(){return 2}});var g=function(a){a=10|a;for(var b="",c="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789",d=0;d<a;d++)b+=c.charAt(Math.floor(Math.random()*c.length));return b};f.defaults.editors.string=f.AbstractEditor.extend({register:function(){this._super(),this.input&&this.input.setAttribute("name",this.formname)},unregister:function(){this._super(),this.input&&this.input.removeAttribute("name")},getValue:function(){if("date-time"===this.format){
// deal with timezone and all date-time issues
// NOTE: This should be the same for both jQuery and html5 based datetime-pickers
var a=this.input.value.trim().replace(" ","T"),b=function(a,b){b=2|b;for(var c=a.toString();c.length<b;)c="0"+c;return c};// seconds
//secfrac
return a&&0!==a.length?a+":"+b(0|this.datetimeInfo.date.getSeconds())+"."+b(0|this.datetimeInfo.date.getMilliseconds(),3)+this.datetimeInfo.timezone:(new Date).toISOString()}return this._super()},setValue:function(a,b,c){var d=this;if("date-time"===this.format){
// deal with timezone and all date-time issues
if(!a)return;a=a.trim();var e="";"Z"===a.substr(a.length-1)?e=a.substr(a.length-1):"+"!==a.substr(a.length-6,1)&&"-"!==a.substr(a.length-6,1)||(e=a.substr(a.length-6)),window.console.log("timezoneInfo str:"+e);var f=new Date(a),g=function(a){var b=""+a.toString();return b.length<2?"0"+b:b},h=f.getFullYear()+"-"+g(f.getMonth()+1)+"-"+g(f.getDate())+"T"+g(f.getHours())+":"+g(f.getMinutes());return this.datetimeInfo={date:f,timezone:e},this.input.value=h,void(window.jQuery&&(window.jQuery("#"+this.input_id).datetimepicker().on("changeDate",function(a){
// Don't allow changing if this field is a template
// Don't allow changing if this field is a template
return a.preventDefault(),a.stopPropagation(),d.schema.template?void(this.value=d.value):(d.is_dirty=!0,d.refreshValue(),void d.onChange(!0))}),window.jQuery("#"+this.input_id).datetimepicker("update",f)))}if((!this.template||c)&&(null===a||"undefined"==typeof a?a="":"object"==typeof a?a=JSON.stringify(a):"string"!=typeof a&&(a=""+a),a!==this.serialized)){
// Sanitize value before setting it
var i=this.sanitize(a);if(this.input.value!==i){this.input.value=i,
// If using SCEditor, update the WYSIWYG
this.sceditor_instance?this.sceditor_instance.val(i):this.epiceditor?this.epiceditor.importFile(null,i):this.ace_editor&&this.ace_editor.setValue(i);var j=c||this.getValue()!==a;this.refreshValue(),b?this.is_dirty=!1:"change"===this.jsoneditor.options.show_errors&&(this.is_dirty=!0),this.adjust_height&&this.adjust_height(this.input),
// Bubble this setValue to parents if the value changed
this.onChange(j)}}},getNumColumns:function(){var a,b=Math.ceil(Math.max(this.getTitle().length,this.schema.maxLength||0,this.schema.minLength||0)/5);return a="textarea"===this.input_type?6:["text","email"].indexOf(this.input_type)>=0?4:2,Math.min(12,Math.max(b,a))},build:function(){var a=this;
// Specific format
