...
//secfrac
return a&&0!==a.length?a+":"+b(0|this.datetimeInfo.date.getSeconds())+"."+b(0|this.datetimeInfo.date.getMilliseconds(),3)+this.datetimeInfo.timezone:(new Date).toISOString()}return this._super()},setValue:function(a,b,c){if("date-time"===this.format){
// deal with timezone and all date-time issues
if(!a)return;a=a.trim();var d="";"Z"===a.substr(a.length-1)?(d=a.substr(a.length-1),a=a.substr(0,a.length-1)):"+"!==a.substr(a.length-6,1)&&"-"!==a.substr(a.length-6,1)||(d=a.substr(a.length-6),a=a.substr(0,a.length-6));var e=new Date(a),f=function(a){var b=""+a.toString();return b.length<2?"0"+b:b},g=e.getFullYear()+"-"+f(e.getMonth()+1)+"-"+f(e.getDate())+" "+f(e.getHours())+":"+f(e.getMinutes());return this.datetimeInfo={date:e,timezone:d},window.console.log("dateStr:"+a),void(this.input.value=g)}if((!this.template||c)&&(null===a||"undefined"==typeof a?a="":"object"==typeof a?a=JSON.stringify(a):"string"!=typeof a&&(a=""+a),a!==this.serialized)){
// Sanitize value before setting it
var h=this.sanitize(a);if(this.input.value!==h){this.input.value=h,
// If using SCEditor, update the WYSIWYG
this.sceditor_instance?this.sceditor_instance.val(h):this.epiceditor?this.epiceditor.importFile(null,h):this.ace_editor&&this.ace_editor.setValue(h);var i=c||this.getValue()!==a;this.refreshValue(),b?this.is_dirty=!1:"change"===this.jsoneditor.options.show_errors&&(this.is_dirty=!0),this.adjust_height&&this.adjust_height(this.input),
// Bubble this setValue to parents if the value changed
this.onChange(i)}}},getNumColumns:function(){var a,b=Math.ceil(Math.max(this.getTitle().length,this.schema.maxLength||0,this.schema.minLength||0)/5);return a="textarea"===this.input_type?6:["text","email"].indexOf(this.input_type)>=0?4:2,Math.min(12,Math.max(b,a))},build:function(){var a=this;
// Specific format
...
// Code editor
if(this.format&&"date-time"===this.format&&window.jQuery&&(window.jQuery("#"+this.input_id).datetimepicker({format:"yyyy-mm-dd hh:ii"}),window.jQuery("#"+this.input_id).datetimepicker().on("changeDate",function(a){
// Don't allow changing if this field is a template
// Don't allow changing if this field is a template
