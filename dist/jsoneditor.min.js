...
var a,b=this.input.value,c=function(a,b){b=2|b;for(var c=a.toString();c.length<b;)c="0"+c;return c};
// RFC3339 compliant:
// seconds
//secfrac
return a=b&&0!==b.length?new Date(b+":"+c(0|this.datetimeInfo.date.getSeconds())+"."+c(0|this.datetimeInfo.date.getMilliseconds(),3)+this.datetimeInfo.timezone):new Date,a.toISOString()}return this._super()},setValue:function(a,b,c){if("date-time"===this.format){
// deal with timezone and all date-time issues
if(!a)return;a=a.trim();var d="";"Z"===a.substr(a.length-1)?d=a.substr(a.length-1):"+"!==a.substr(a.length-6)&&"-"!==a.substr(a.length-6)||(d=a.substr(a.length-6));var e=new Date(a),f=function(a){var b=""+a.toString();return b.length<2?"0"+b:b};return this.input.value=e.getFullYear()+"-"+f(e.getMonth()+1)+"-"+f(e.getDate())+"T"+f(e.getHours())+":"+f(e.getMinutes()),void(this.datetimeInfo={date:e,timezone:d})}if((!this.template||c)&&(null===a||"undefined"==typeof a?a="":"object"==typeof a?a=JSON.stringify(a):"string"!=typeof a&&(a=""+a),a!==this.serialized)){
// Sanitize value before setting it
var g=this.sanitize(a);if(this.input.value!==g){this.input.value=g,
// If using SCEditor, update the WYSIWYG
this.sceditor_instance?this.sceditor_instance.val(g):this.epiceditor?this.epiceditor.importFile(null,g):this.ace_editor&&this.ace_editor.setValue(g);var h=c||this.getValue()!==a;this.refreshValue(),b?this.is_dirty=!1:"change"===this.jsoneditor.options.show_errors&&(this.is_dirty=!0),this.adjust_height&&this.adjust_height(this.input),
// Bubble this setValue to parents if the value changed
this.onChange(h)}}},getNumColumns:function(){var a,b=Math.ceil(Math.max(this.getTitle().length,this.schema.maxLength||0,this.schema.minLength||0)/5);return a="textarea"===this.input_type?6:["text","email"].indexOf(this.input_type)>=0?4:2,Math.min(12,Math.max(b,a))},build:function(){var a=this;
// Specific format
